...
export const FiltersChoices: FC<FiltersChoicesProps> = ({choices}) => {
    ...
    return (
        <div className={styles.choices}>
            <Chip className={styles.choice}>Рейтинг: {rating[0]} - {rating[1]}</Chip>
            <Chip className={styles.choice}>Года производства: {year[0]} - {year[1]}</Chip>
            <Chip className={styles.choice}>Жанр: {genres.label}</Chip>
            <Chip className={styles.choice}>Год выхода: {sort === '-1' ? 'Сначала новые' : 'Сначала старые'}</Chip>
        </div>
    )
}
